...
class Validator {
	...
	private $msg = array(	   
    	...
    	"rc4"  			=> "The :attribute does not pass validation (rc4)",
    	"filtermsg"	=> "Content is illegal",
	);
	...
	protected function check($attribute, $rule)
	{
		...
		$value = func::array_get($this->attributes, $attribute);
	...
	{
		$other = $parameters[0];
		$other_value = func::array_get($this->attributes, $other);		
	...
	{
		if ( ! is_array($value) or func::array_get($value, 'tmp_name', '') == '') return true;
	...
	{		
		$custom = $this->msg[$rule] ? $this->msg[$rule] : $attribute.' need '.$rule;
